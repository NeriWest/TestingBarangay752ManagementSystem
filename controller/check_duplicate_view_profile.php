...
try {
    ...
    if ($field === 'email') {
        ...
        $username_table = 'accounts';
        $username_column = 'username';
        $username_stmt = $conn->prepare("SELECT COUNT(*) as count FROM $username_table WHERE $username_column = ?");
        if (!$username_stmt) {
            throw new Exception('Username prepare failed: ' . $conn->error);
        }
        $username_stmt->bind_param('s', $value);
        if (!$username_stmt->execute()) {
            throw new Exception('Username execute failed: ' . $username_stmt->error);
        }
        $username_result = $username_stmt->get_result();
        $username_row = $username_result->fetch_assoc();
        $username_count = $username_row['count'];
        error_log("Username check: value=$value, count=$username_count");
        $exists = $username_count > 0;
        $message = $exists ? 'Cellphone number already exists, try another one.' : '';
        $username_stmt->close();
    } else {
