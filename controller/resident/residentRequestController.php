...
require '../../model/resident/residentCertificateModel.php';
...
$requests = $residentCertificateRequestModel->showCertificateRequestsWithPagination($conn, $offset, $recordsPerPage, $residentId);

// Get the total number of certificate requests (for pagination calculation)
$totalCertificateRequests = $residentCertificateRequestModel->getTotalCertificateRequests($conn, $residentId);
...
$typeOfPayment = $residentCertificateRequestModel->getTypeOfPayment($conn);
$typeOfDocument = $residentCertificateRequestModel->getTypeOfDocument($conn);
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert the request
    if ($residentCertificateRequestModel->insertRequest($conn, $requestData)) {
        // $residentActivityLogModel = new ActivityLogModel();
        ...
        // $residentActivityLogModel->recordActivityLog($conn, $accountId, $module, $activity, $dateCreated, date('H:i:s'), $description);
        // session_start();
        $_SESSION['message'] = "New request has been successfully added.";
        header('Location: residentRequestController.php');
        exit;
    } else {
        // Handle insert error
        // session_start();
...
require '../../public/views/resident/residentRequest.php';
