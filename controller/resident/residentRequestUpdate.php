...
$residentCertificateRequestModel = new CertificateRequestModel();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['proof_of_payment']) && $_FILES['proof_of_payment']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['proof_of_payment']['tmp_name'], $filePath)) {
            ...
            header('Location: residentPermitController.php');
            exit;
        }
    } else {
    ...
    // Update the request
    if ($residentCertificateRequestModel->updateRequest($conn, $requestId, $requestData)) {
        ...
        $residentActivityLogModel->recordActivityLog($conn, $accountId, $module, $activity, $description);
