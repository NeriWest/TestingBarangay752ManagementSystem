...
$complaints = $reportModel->showComplaintsWithPagination($conn, $offset, $recordsPerPage);

// Get the total number of accounts (for pagination calculation)
$totalComplaints = $reportModel->getTotalComplaints($conn);

// Calculate total pages
$totalPages = ceil($totalComplaints / $recordsPerPage);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $new_complaint_id = $reportModel->createComplaint($conn, $blotterData, $evidenceData);
    if ($new_complaint_id) {
        $adminActivityLogModel = new ActivityLogModel();
        $module = "Blotter Management";
        $activity = "Added a new blotter case with ID: " . $new_complaint_id . ".";
