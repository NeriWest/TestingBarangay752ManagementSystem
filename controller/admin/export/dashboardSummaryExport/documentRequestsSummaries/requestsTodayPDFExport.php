...
class PDFWithHeaderFooter extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 10, 'TODAY\'S PENDING REQUESTS LIST', 0, 1, 'C');
        ...
        $this->Cell(0, 6, 'New Requests: ' . $GLOBALS['totalRequestsToday'], 0, 1, 'C');
        $this->Ln(8);
...
$requests = $adminResidentModel->GETrequestsToday($conn, 0, 0); // Get data for today (no pagination needed here)
$GLOBALS['totalRequestsToday'] = is_array($requests) ? count($requests) : 0; // Safe count
$conn->close();
...
$pdf->Output('Requests_Today_PDF_Export_('.date('m-d-Y').').pdf', 'I');
?>
