...
class PDFWithHeaderFooter extends FPDF {
    ...
    function Header() {
        // Logos
        $this->Image('../../../../../img/Barangay Logo.png', 69, 5, 20);
        $this->Image('../../../../../img/lunsgodNgMayNilaLogo.png', 209, 5, 20);

        // Title
        $this->SetFont('Arial', 'B', 16);
        $this->Cell(0, 10, 'TODAY\'S REQUESTED CERTIFICATES LIST', 0, 1, 'C');
        ...
        $this->Cell(0, 6, 'New Certificates: ' . $GLOBALS['newCertificates'], 0, 1, 'C');
        $this->Ln(8);
...
$requests = $adminResidentModel->GETcertificatesToday($conn, 0, 0); // Get data for today (no pagination needed here)
$GLOBALS['newCertificates'] = is_array($requests) ? count($requests) : 0; // Safe count
$conn->close();

// Setup headers
$headers = [
    ['width' => 15, 'title' => 'Request ID'],
    ['width' => 55, 'title' => 'Type of Document'],
...
$pdf->Output('New_Certificates_Today_PDF_Export_('.date('m-d-Y').').pdf', 'I');
?>
